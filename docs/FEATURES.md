...
| WEB-AUTH-KAKAO | 카카오 로그인 | `app.js`, `auth-flow.js`, Supabase Kakao provider | `tests/web/auth-flow.test.mjs`, `tests/release/web-static.test.mjs`, `tests/release/web-kakao.md` | 실로그인은 CI에 없음. Kakao Developers·Provider 설정 후 `web-kakao.md`. 이메일은 비즈앱 `account_email` |
...
| MCP-TOOLS | 공간·할일·아이디어 MCP 도구 | `mcp-server/server.mjs` | `tests/mcp/server.test.mjs` | 임의 SQL 없음. 계약 테스트는 지금 가능 |
| MCP-HTTP | HTTP `POST /mcp`, Bearer, Origin 검사 | `server.mjs --http` | `tests/mcp/server.test.mjs` | `127.0.0.1`만. 인터넷 직접 공개 안 함. 실서버 확인은 MCP-PC-RUN |
| MCP-PC-SERVER | PC가 서버, Cursor가 HTTP 클라이언트 | `.cursor/mcp.json`, `scripts/*.ps1` | `tests/release/mcp-http.md` (나중) | 스크립트·설정은 있음. `.env` 실값·실조회는 MCP-PC-RUN |

## in_progress

| ID | 기능 | 다음에 할 일 | 테스트 |
|---|---|---|---|
| MCP-PC-RUN | 이 PC에서 HTTP 서버 실가동·클라이언트 조회 | **나중에 테스트.** `.env` 채우기, `http:install`, Cursor/Grok 재시작 후 `list_spaces` | `tests/release/mcp-http.md`. 서버가 떠 있으면 `npm run test:release` |
...
2. 자동 테스트가 있으면 `tests/`에 두고 이 표의 테스트 칸에 경로를 적는다. 사람이 눌러 보는 항목은 `tests/release/`에 체크리스트로 남긴다. 실계정·카카오 동의 화면은 CI에 넣지 않는다.
